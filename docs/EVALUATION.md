...
The repository has 58 deterministic tests covering admission, temporal versioning, conflict handling, human review, abstention, tenant/role boundaries, PII/credential controls, API authentication, DeepSeek provider switching and key-safe failures, governed prompt semantics, audit privacy, metrics, spans, public-evaluation code and the local Agent path.
